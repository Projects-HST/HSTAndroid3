package com.skilex.skilexserviceperson.customview;

/**
 * Created by Narendar on 14/10/17.
 */
...
public class CustomOtpEditText extends LinearLayout {
    ...
    private void init(AttributeSet attrs) {
        TypedArray styles = getContext().obtainStyledAttributes(attrs, R.styleable.CustomOtpEditText);
        LayoutInflater mInflater =
                (LayoutInflater) getContext().getSystemService(Context.LAYOUT_INFLATER_SERVICE);
        mInflater.inflate(R.layout.otp_edittext, this);
    ...
    private void styleEditTexts(TypedArray styles) {
        int textColor = styles.getColor(R.styleable.CustomOtpEditText_android_textColor, Color.BLACK);
        int backgroundColor =
...
//            mOtpFourField.getBackground().mutate().setColorFilter(textColor, PorterDuff.Mode.SRC_ATOP);
    ...
    private void setEditTextInputStyle(TypedArray styles) {
        int inputType =
                styles.getInt(R.styleable.CustomOtpEditText_android_inputType, EditorInfo.TYPE_TEXT_VARIATION_NORMAL);
